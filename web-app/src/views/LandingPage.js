...
import contentfull from 'config/contentfullConfig';
...
function LandingPage(props) {
	...
	const { getEstimate, clearEstimate, addBooking, clearBooking, MinutesPassed, GetDateString } = api;
	...
	return (
		<div>
							...
							<GridItem xs={6} sm={6} md={3} lg={3}>
								<FormControl style={{ width: '100%' }}>
									<Select
										...
										className={
											carType === language.select_car ? classes.inputdimmed : classes.input
										}
									>
										...
										{cartypes.map((car) => (
											<MenuItem key={car.name} value={car.name}>
												<img src={car.image} className={classes.carphoto} alt="car types" />
												{car.name}
											</MenuItem>
										))}
									</Select>
			...
			<AlertDialog open={bookingdata.booking ? true : false} onClose={handleBookingAlertClose}>
				{bookingdata.booking ? language.booking_success + bookingdata.booking.booking_id : null}
			</AlertDialog>
			<AlertDialog open={bookingdata.error.flag} onClose={handleBookingErrorClose}>
				{bookingdata.error.msg}
			</AlertDialog>
			<AlertDialog open={estimatedata.error.flag} onClose={handleEstimateErrorClose}>
...
export default withPage(LandingPage, 'homepage');
