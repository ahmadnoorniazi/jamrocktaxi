...
import { fetchBookings, updateBooking, cancelBooking } from './actions/bookinglistactions';
import { fetchCancelReasons, editCancellationReason } from './actions/cancelreasonactions';
...
import { fetchNotifications, editNotifications, sendNotification } from './actions/notificationactions';
import { fetchPromos, editPromo } from './actions/promoactions';
import { addUser, fetchUsers, fetchDrivers, editUser } from './actions/usersactions';
import { fetchSettings, editSettings, clearSettingsViewError } from './actions/settingsactions';
import { fetchPaymentMethods, addToWallet, updateWalletBalance, clearMessage } from './actions/paymentactions';
import {
	updateTripPickup,
	updateTripDrop,
	updateTripCar,
...
import { fetchBookingLocations, stopLocationFetch } from './actions/locationactions';
import { fetchChatMessages, sendMessage, stopFetchMessages } from './actions/chatactions';
...
import { selectedState } from './actions/selectedStateActions';
import { setCartData } from './actions/cartActions';
	...
	if (!app.apps.length) {
		...
		firebase = {
			...
			api: {
				...
				pageLoad: (pageKey) => (dispatch) => fetchPage(pageKey)(dispatch),
				selectedState: (data) => (dispatch) => selectedState(data)(dispatch),
				setCartData: (data, type) => (dispatch) => setCartData(data, type)(dispatch)
			}
		};
	}
